...
dirichlet.plot.data <- expand.grid( 
  lnratio = seq(min(sub.org.long$lnratio), max(sub.org.long$lnratio), length.out = 30),
...
predicted.data <- predicted.data%>%rename(
  pom = V1, 
  sa = V2,
  sc = V3,
  doc = V4)
...
```{r }
library(gridExtra)
grid.arrange(pctc.species, pctn.species, hwec.species, ln.species, ncol = 2)
